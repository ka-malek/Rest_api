...
router.get("/user",async(req,res)=>{
    try {
        ...
        res.status(200).json({response:result,msg:"geting Contacts with Success"})
    } catch (error) {
...
router.get('/user/:id',async(req,res)=>{
    try {
        ...
        !result ? res.status(400).send({msg:"there is no contact with this id"})
        : res.status(200).send({response:result, msg:"getting Contact  with Success"})
    } catch (error) {
...
router.put("/user/:id",async(req,res)=>{
    try {
        ...
        : res.status(200).send({response:test,msg:"Update Successfully"});
    } catch (error) {
        res.status(500).send({msg:"can not get contact"});
    }
})
